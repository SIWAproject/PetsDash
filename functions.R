...
diff_geneexp <-
  function(df_gene, GroupRef, variable="KitID", print=FALSE) {
    ...
    if (GroupRef %in% df_gene[[variable]]){
      refs <-
        df_gene[df_gene[[variable]] == GroupRef, ] %>% group_by(SampleLocation) %>% summarise(
...
aggregate_taxa_siwa <-
  function(phylo_relative, taxa_level) {
    OTUS <- as.data.frame(otu_table(phylo_relative))
    OTUS$otu <- rownames(OTUS)
    taxa <- as.data.frame(tax_table(phylo_relative)) 
